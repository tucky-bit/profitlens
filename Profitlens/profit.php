                ...
                <tr class="trash-row" data-year="<?= $rowYear ?>" data-total="<?= floatval($s['total']) ?>">
                    ...
                    <td style="white-space:nowrap;">
                        <button type="button" class="btn-restore"
                            onclick="openRestoreSingleModal(<?= $s['id'] ?>, '<?= htmlspecialchars(addslashes($s['product_name'] ?? 'N/A'), ENT_QUOTES) ?>', '<?= $dispDate ?>')"
                        >Restore</button>
...
<!-- ── Restore Single Sale Modal ── -->
<div class="delete-modal-overlay" id="restore-single-modal-overlay" onclick="handleRestoreSingleOverlayClick(event)">
    <div class="delete-modal">
        <div class="delete-modal-icon" style="background:#e8f5e9;">♻️</div>
        <h3>Restore This Sale?</h3>
        <p>You're about to restore</p>
        <p><span class="sale-name" id="restore-single-sale-name">—</span></p>
        <p id="restore-single-sale-date" style="font-size:12px;color:#888;margin-bottom:4px;font-family:'Poppins',sans-serif;"></p>
        <p class="cannot-undo">It will reappear in your active <strong>Sales History</strong>.</p>
        <div class="delete-modal-actions">
            <button class="btn-cancel" onclick="closeRestoreSingleModal()">Cancel</button>
            <a id="restore-single-confirm-link" href="#" class="btn-confirm-delete" style="background:#28a745;">Yes, Restore</a>
        </div>
    </div>
</div>
...
// ── Restore Single Sale Modal ──
function openRestoreSingleModal(id, productName, saleDate) {
    document.getElementById('restore-single-sale-name').textContent = productName;
    document.getElementById('restore-single-sale-date').textContent = 'Date: ' + saleDate;
    document.getElementById('restore-single-confirm-link').href = 'profit.php?action=restore&id=' + id + '&year=<?= $view_year ?>';
    document.getElementById('restore-single-modal-overlay').classList.add('open');
    document.body.style.overflow = 'hidden';
}
function closeRestoreSingleModal() {
    document.getElementById('restore-single-modal-overlay').classList.remove('open');
    document.body.style.overflow = '';
}
function handleRestoreSingleOverlayClick(e) {
    if (e.target === document.getElementById('restore-single-modal-overlay')) closeRestoreSingleModal();
}
